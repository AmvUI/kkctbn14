...
//    cvLine(result, cvPoint(0, target + 60), cvPoint(300, target + 60), cvScalar(255, 0, 0), 2);
    ...
    double              pid = p + i + d;
	 ...
	 rem(2);
		...
		if (motorKiri > 1500) {motorKiri = 1500;}
		if (motorKanan > 1500) {motorKanan = 1500;}
		// usleep(185000);
       serialWrite(motorKiri, motorKanan, nozzleState,19);
       printf("Throttle : %s %f %d %d %d\n",movementStat, pid,  motorKiri, motorKanan,nozzleState);
