#include <vector> include <opencv/cv.h> include <opencv/cvaux.h> 
#include <opencv/highgui.h> include <iostream> include <list> 
#include <opencv2/video/background_segm.hpp> include 
#<opencv2/legacy/blobtrack.hpp> include 
#<opencv2/highgui/highgui.hpp> include 
#<opencv2/imgproc/imgproc_c.h> include <opencv2/core/core.hpp> 
#include <opencv2/video/tracking.hpp> include <math.h> include 
#<unistd.h> include <stdio.h> include <stdlib.h> include 
#<sys/ioctl.h> include <fcntl.h> include <termios.h> include 
...
int              last_known_x   = 580; // last known position of the target.
int		 x	=	580;
...
int              sat_level 		= 69;  //aslinya 15
int              target 		= 580; // posisi target
...
static double    cp             = 2;
static double    ci             = 0.6;
static double    cd             = 0.2;
...
//  cvMoveWindow("Track", 646, 0);
  ...
  for (int n = 0;; ++n) {
	...
	cvSetImageROI(framein, cvRect(0,330, 640, 100));
...
//    cvShowImage("Track", result);
  ...
  //karna mencrop frame, thresholding jadi lebih cepat
  // serialnya gak kuat jadi harus dilamban.	
  //  usleep(50000);
 if (x < 1 || x > 640) {
      x = last_known_x;
      }
